...
public class ItemStoneBow extends ItemBow
{
    ...
    public ItemStoneBow(int par1, int i)
    {
        super(par1);
        maxStackSize = 1;
        setMaxDamage(484);
    ...
    {
            if(player.isUsingItem() && usingItem.itemID == iDiamondhunter.StoneBow.shiftedIndex)
            {
                if (usingItem != null && usingItem.getItem().shiftedIndex == iDiamondhunter.StoneBow.shiftedIndex)
                {
                    int k = usingItem.getMaxItemUseDuration() - useRemaining;
                    if (k >= 18) return iDiamondhunter.StoneBow_4;
                    if (k > 13) return iDiamondhunter.StoneBow_3;
                    if (k > 0) return iDiamondhunter.StoneBow_2;
                }
            }
            else 
            {
            iconIndex = iDiamondhunter.StoneBow_1;
            }
            return iconIndex;
    }
    ...
    public void onPlayerStoppedUsing(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer, int par4)
    {
        ...
        if(cInv.itemID == iDiamondhunter.StoneBow.shiftedIndex)
        {
        	return;
        }
        else
