package iDiamondhunter.morebowsmod.common;
...
public class ItemIronBow extends ItemBow
{
    ...
    public ItemIronBow(int par1, int i)
    {
        super(par1);
        maxStackSize = 1;
        setMaxDamage(550);
    ...
    {
            if(player.isUsingItem() && usingItem.itemID == iDiamondhunter.IronBow.shiftedIndex)
            {
                if (usingItem != null && usingItem.getItem().shiftedIndex == iDiamondhunter.IronBow.shiftedIndex)
                {
                    int k = usingItem.getMaxItemUseDuration() - useRemaining;
                    if (k >= 16) return iDiamondhunter.IronBow_4;
                    if (k > 11) return iDiamondhunter.IronBow_3;
                    if (k > 0) return iDiamondhunter.IronBow_2;
                }
            }
            else 
            {
    ...
    public void onPlayerStoppedUsing(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer, int par4)
    {
        ...
        if (flag || par3EntityPlayer.inventory.hasItem(Item.arrow.shiftedIndex))
        {
            ...
            EntityArrow var8 = new EntityArrow(par2World, par3EntityPlayer, f * 2.1F);
        ...
        if(cInv.itemID != iDiamondhunter.IronBow.shiftedIndex)
        {
        	this.iconIndex = iDiamondhunter.IronBow_1;
        }
    }
